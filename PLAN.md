...
- **Network neighbour on Gavin's LAN / coffee-shop Wi-Fi** — capability: send TCP to Gavin's IP. Goal: read fixtures / kick off generation runs that exhaust Gavin's Claude Code rate limits / probe the API. Mitigation: the FastAPI server MUST bind `127.0.0.1`, never `0.0.0.0`, never `::`. The launcher refuses to start if the configured `BIND_HOST` is anything else.
- **Browser tab on a website Gavin visits** — capability: cross-origin requests / DNS rebinding to `localhost`. Goal: invoke admin endpoints (re-import, build-static), spawn Claude Code worktree subagents in a tight loop (subscription rate-limit DoS), exfiltrate ranking data. Mitigation: CORS allow-list to the local Vite dev origin only; reject `Host:` headers that aren't `127.0.0.1` / `localhost` (DNS-rebinding defence); state-changing endpoints require a same-origin custom header (`X-Requested-By: bible-study-ui`) that browsers won't add cross-origin without preflight.
- **Future maintainer pushing a public branch** — capability: accidental commit of `.env`, SQLite snapshot, or `dist/` containing local-machine paths or fixture URLs. Goal: not malicious — but the result is information leakage. Mitigation: gitignore + pre-commit secret scanner (defensive — there should be no secrets in this repo to begin with; see Data exposure below), `dist/` audit step.
...
- **Claude Code subagent itself** — capability: return adversarial text in a candidate (the subagent runs in a sandboxed scratch worktree but its stdout becomes candidate text). Goal: trick downstream rendering / export. Mitigation: candidate text is treated as untrusted user content — never executed, never interpolated into HTML without React's default escaping, never used to compose a follow-up prompt without quoting.
- **Gavin himself, accidentally** — capability: hit "Build static site," `git add dist/`, `make dev` on a public network, run with `BIND_HOST=0.0.0.0` to test from his phone, fan out a runaway batch that exhausts the Claude Code subscription rate limit. Goal: none — but each of these leaks or self-DoSes. The single-user framing does not exempt us from defending against accidental footguns.
...
- **No API keys live in this repo or its bundle.** Generation runs through the local `claude` CLI (CLAUDE.md §Generation mechanism); the trust boundary for Claude Code is its own OS-level session (Gavin's logged-in subscription), not an env var the FastAPI process reads. There is no `ANTHROPIC_API_KEY`, no `.env` secret to redact, no key to strip out of the React bundle, and no key-rotation procedure to document. The `api/claude/` worktree spawner inherits the calling process's environment so the `claude` subprocess sees whatever Claude Code itself looks for, but nothing in this repo's code paths reads, logs, or serialises a key. The legacy build-time grep guards on `dist/` for `sk-ant-` / `ANTHROPIC_API_KEY` strings are kept as defence-in-depth (see Build-time guards below) — they should never fire, and a positive hit means something has been added to the codebase that shouldn't be there.
...
- **Static-site export sanitization (`dist/`):**
  - No secrets (defence-in-depth assertion: bundle contains no string matching `/sk-ant-[A-Za-z0-9_-]{20,}/`, no occurrence of `ANTHROPIC_API_KEY`). No such strings should exist in source either; this guard is paranoia, not policy.
  - No server-side generation code (assertion: `dist/` contains no reference to any module under `api/claude/`, no reference to `api/runs/runner.py`).
...
- **Logging discipline:**
  - Structured logs (JSON to stdout). Never log: prompt-template fragments in full, full subagent stdout (may be large), `Authorization` / `X-Api-Key` / any header matching `*key*` (defensive — no such headers should be reaching us), `Set-Cookie`, `.env` contents, `payload_json` blobs, the absolute path of a worktree (basename only is fine).
  - Do log: route, status, duration, `run_id`, `sentence_id`, `candidate_id`, `ordinal`, `error_code` (the coarse enum from `generation_run_items.error_code`).
  - On worktree-subagent error, log only `error_code` + a hash of the failing prompt's `snapshot_hash`, not the prompt body. The `error_message` shown in the UI may quote the subagent's stderr / unparseable stdout verbatim (Designer Flow 4 error state) — that's fine for the UI but the log keeps a hash, not the text, to avoid disk-leaking content that may quote prompt material.
...
- **`claude` CLI version is recorded, not pinned by this repo.** The CLI is installed and updated by Claude Code itself; we capture `claude --version` at run-creation time and store it as the candidate's `model` provenance string (e.g. `claude-code-2.1.126`) so a generated candidate is traceable to the CLI version that produced it.
...
- **Secret rotation:** Not applicable — this repo holds no Anthropic credentials. Claude Code's own session is rotated through Claude Code's tooling, outside this project. CI does not run real Claude generation; tests substitute a fake spawner (`api/tests/fakes.py`) for the worktree spawner.
...
- **No telemetry, no auto-update, no remote logging, no outbound network calls from this repo.** Generation goes through the local `claude` CLI subprocess (whatever network calls Claude Code itself makes are outside this project's responsibility). Fixture refresh is a deliberate manual operation (`make fetch-fixtures`).

### Abuse, rate-limiting, and subscription safety

There is no per-token billing this repo can run up — generation uses Gavin's already-paid Claude Code subscription. The realistic "harm" vector shifts to **Gavin (or a runaway script, or an open browser tab) exhausting his Claude Code subscription's rate limits / saturating the local machine's process count by spawning too many worktree subagents**. The mitigations are the same shape as before but the units are worktrees and wall-clock seconds, not dollars:

- **Per-run hard cap:** `CreateRunRequest` is rejected if the resolved sentence list exceeds `MAX_WORKTREES_PER_RUN` (default 200, env-overridable; `max_sentences_per_run` is kept as a backwards-compatible alias). One chapter (~100 sentences) is the realistic max; "all unranked red-letter" is bounded by Matthew's red-letter count.
- **Daily run-count cap:** the runner counts `generation_runs` rows whose `created_at` is within the last 24h (UTC) and rejects new runs with `code: "max_runs_per_day_exceeded"` once that count would reach `MAX_RUNS_PER_DAY` (default 100, env-overridable). The aggregate is a pure SQL `COUNT(*)` over `idx_gr_created_at`. Replaces the earlier dollar-based daily cap because there is no dollar to aggregate; runs-per-day is the proxy for "Gavin's subscription is being thrashed."
- **In-flight concurrency cap:** `MAX_CONCURRENT_WORKTREES` (default 2, env-overridable) bounds how many sentences within a run can have a live worktree subagent at once. Implemented as an `asyncio.Semaphore` around the per-sentence task body. Prevents a runaway batch from spawning hundreds of `claude` subprocesses in parallel — a local-machine resource concern as well as a Claude Code rate-limit concern.
- **Per-sentence wall-clock cap:** `MAX_WALL_CLOCK_SECONDS_PER_SENTENCE` (default 300) bounds each individual subagent invocation; on timeout the process group is killed and the item is marked `failed` with `error_code='timeout'`. Prevents a wedged subagent from holding a semaphore slot forever.
...
- **Cancel is honoured promptly:** `POST /runs/{id}/cancel` flips the run's status; the dispatcher checks the flag between sentences and stops dispatching. In-flight subagent subprocesses drain (Architect's design: don't throw away work already underway — the result still persists).
- **No auto-retry of subagent failures.** "Retry this sentence" is an explicit user click that creates a new single-sentence run — no exponential-backoff loop that quietly multiplies subscription pressure.

**`claude_cli_unavailable` short-circuit:** at run-creation time the runner calls `claude --version`; if the CLI isn't on `PATH` or the subscription session isn't logged in, the run is rejected with HTTP 400 `code='claude_cli_unavailable'` — no `generation_runs` row is written. This stops a runaway loop from accumulating unstartable runs.
...
- **`make build-static` refuses to write `dist/`** if any of the following grep checks fail:
  - `dist/` contains `sk-ant-` or any other configured secret prefix (defence-in-depth — there should be no key in source either).
...
- **CI check:** the static-build authoring strip-out is verified by greppping `dist/` for known authoring identifiers and write-endpoint URLs (`/admin/`, `/runs`, `If-Match`); zero hits is required. This is the load-bearing test that proves the Vite tree-shake is real (the legacy "synthetic API key" variant is moot — there is no key for the build to leak).
...
| API key leakage from this repo | None to leak — no API keys are read or stored by this repo (Claude Code's own session is the trust boundary) |
...
| Subscription rate-limit / runaway worktree fan-out | Per-run cap (`MAX_WORKTREES_PER_RUN`), daily run-count cap (`MAX_RUNS_PER_DAY`), concurrency cap (`MAX_CONCURRENT_WORKTREES`), per-sentence wall-clock cap (`MAX_WALL_CLOCK_SECONDS_PER_SENTENCE`), server-enforced (not UI-only) |
| Browser-tab CSRF | Same-origin custom-header requirement on all writes |
| Accidental secret commit | Gitignore + pre-commit secret scanner (defensive — there should be no secrets to commit) |
| Static-site code leakage | Build-step deny-list grep; `dist/` written from explicit allow-list of paths |
...
5. **`test_no_secret_strings_in_responses`** — hit every GET endpoint, assert no string matching `sk-ant-` or `ANTHROPIC_API_KEY` appears in any response body. Defensive — no such string should exist anywhere in the codebase to begin with.
6. **`test_no_secret_strings_in_logs`** — exercise a generation run (with the fake spawner), capture stdout, assert no `sk-ant-` / `ANTHROPIC_API_KEY` appears.
...
8. **`test_static_build_no_secret_strings`** — run the static build, grep `dist/` for `sk-ant-` and `ANTHROPIC_API_KEY`; must be zero hits. (Replaces the legacy "synthetic API key" test — there is no key for the build to leak; this test asserts the absence of a hypothetical leak vector as defence-in-depth.)
...
12. **`test_style_prompt_compatible_source_sets_validated_at_run_creation`** — actual test: call `POST /runs` with a `(prompt, source_set_id)` pair where the prompt's front-matter does not include the source set; assert the run is rejected before any subagent is spawned. Lives in `api/tests/test_runs_create.py`.
13. **`test_create_run_rejects_when_claude_cli_unavailable`** — actual test: with the spawner reporting `cli_available()=False`, post a run, assert 400 `code='claude_cli_unavailable'`, no `generation_runs` row written, no subagent spawned. Lives in `api/tests/test_runs_create.py`. (Replaces the legacy `test_run_rejected_when_estimated_cost_exceeds_cap` — there is no dollar cap to enforce.)
14. **`test_create_run_rejects_when_max_runs_per_day_exceeded`** — actual test: pre-seed `MAX_RUNS_PER_DAY` rows in `generation_runs` within the last 24h, post a new run, assert 400 `code='max_runs_per_day_exceeded'`. Lives in `api/tests/test_runs_create.py`. (Replaces `test_daily_cost_cap_enforced`.)
15. **`test_create_run_rejects_when_sentences_exceeds_max_per_run`** — actual test: `CreateRunRequest` with a scope expanding past `MAX_WORKTREES_PER_RUN` is rejected at run-creation time, before any sentence is dispatched. Lives in `api/tests/test_runs_create.py`.
16. **`test_concurrency_cap_enforced`** — start a run; assert never more than `MAX_CONCURRENT_WORKTREES` simultaneous worktree spawns at any moment (observable via the fake spawner's in-flight counter).
17. **`test_cancel_stops_dispatch`** — start a run, immediately POST cancel, assert the count of dispatched sentences ≤ `MAX_CONCURRENT_WORKTREES` (in-flight drain) and remaining items are `cancelled`.
...
20. **`test_adversarial_subagent_output_rendered_safely`** — fake spawner returns `<script>alert(1)</script>` as candidate text; render it through the rank UI's component (RTL test); assert it appears as escaped text, not an executed script.

**Eval-seed entries** (for the eval harness CLAUDE.md describes; the `X-Twin-*` pattern is waived in v1 because there is no HTTP outbound from this repo — see Architect §Trade-offs):

- `eval_no_subagent_spawned_when_run_creation_rejected_for_rate_cap`: post a run that exceeds `MAX_RUNS_PER_DAY`, assert the fake spawner's `spawned_count` is 0 (no subagent was invoked).
- `eval_no_subagent_spawned_when_claude_cli_unavailable`: post a run when the fake spawner reports `cli_available()=False`, assert HTTP 400 `claude_cli_unavailable` and `spawned_count == 0`.
...
- `eval_dist_passes_secret_scan`: run a fresh build, run the secret scanner against `dist/`, assert clean. (No synthetic key needs to be planted — the test is a sanity check that nothing accidentally pulled an outside secret into the bundle.)

### Open concerns (need user decision)

- **`MAX_WORKTREES_PER_RUN` / `MAX_RUNS_PER_DAY` / `MAX_CONCURRENT_WORKTREES` / `MAX_WALL_CLOCK_SECONDS_PER_SENTENCE` defaults** — currently `200 / 100 / 2 / 300` (per `api/settings.py`). Gavin should confirm these match the behaviour he wants from his Claude Code subscription; all four are env-overridable so this is not a blocker.
...
- **Daily run-count window** — 24h rolling vs. calendar-day UTC. Rolling is friendlier (the cap doesn't reset at a fixed wall-clock moment); calendar-day is simpler to reason about. Picking rolling unless Gavin objects.
...
The Security section already covered: `127.0.0.1` bind-refuse-otherwise, `Origin`/`Host`/`X-Requested-By` middleware, no-API-key-in-this-repo invariant, `dist/` strip-out grep guards, fixture SHA-256 mismatch as `fixture_hash_mismatch`, per-run / per-day / concurrency / wall-clock caps on worktree subagents, `claude_cli_unavailable` short-circuit, no auto-retry of subagent failures. This section does not re-litigate any of those — they are accepted preconditions. The work here is **what happens after a fault, restart, or stale write**: how the in-process runner survives a crash, how SSE clients reconverge on the truth, how OCC actually serializes under SQLite WAL, how the fixture import reaches an all-or-nothing boundary, and how each invariant is pinned by a test.

### Failure modes

- **Server crash / `make dev` restart mid-run**: in-process async runner is gone; `generation_runs` rows in `running` and any `generation_run_items` in `pending` / `running` are now orphaned (no live owner). Symptom: UI shows a "running" run that will never produce events. Blast radius: one run. In-flight `claude` subprocesses are detached from the parent on POSIX (`preexec_fn=os.setsid`) — on a clean `make dev` shutdown they're SIGTERM'd via the process group; on a crash they may continue running locally until their `MAX_WALL_CLOCK_SECONDS_PER_SENTENCE` timeout fires, at which point they exit and leave their worktree behind. `sweep_stale_worktrees` runs at startup to clean any orphaned `data/worktrees/{run_id}/` directories left over from a prior crash.
- **Worktree-spawn failure modes** — surfaced via `error_code` on `generation_run_items`. The worktree runner's typed codes:
  - `claude_cli_unavailable` (run-creation-time only): `claude --version` failed (CLI not on PATH or subscription session not logged in). The run is rejected with HTTP 400 before any row is written. Asserted by `test_create_run_rejects_when_claude_cli_unavailable` in `api/tests/test_runs_create.py`.
  - `timeout`: the subprocess exceeded `MAX_WALL_CLOCK_SECONDS_PER_SENTENCE`. Process group is killed (`os.killpg(SIGKILL)` on POSIX); the item is marked `failed` with `error_code='timeout'`. Asserted by `test_timeout_marks_item_failed_with_timeout_error_code` in `api/tests/test_runs_failure_modes.py`.
  - `invalid_response`: subagent stdout was empty, exceeded `_MAX_STDOUT_BYTES` (256 KiB), or — for `output_format: json` prompts — failed to parse as JSON after fence-stripping. The item is marked `failed`; `error_message` carries the parser's complaint (truncated to `MAX_ERROR_MESSAGE_BYTES`, default 2 KiB, so a misbehaving subagent can't bloat the row). Asserted by `test_invalid_response_marks_item_failed`.
  - `internal`: catch-all for unexpected failure paths (git worktree creation failure, subprocess spawn failure, unexpected SQLite error mid-write). The runner does **not** auto-retry — "Retry this sentence" is an explicit user click. Other sentences in the same run continue. Asserted by `test_internal_error_marks_item_failed`.
  - `disk_full`: SQLite raised `database or disk is full` during the per-sentence transaction. The item is marked `failed` with this typed code; the run continues until **N consecutive `disk_full` failures** trip the runner-level fault (see "Disk full during `claude_candidates` insert" below).
- A run finishes in `completed` with mixed item statuses — that is a normal terminal state, not `failed`. The run-level `status=failed` is reserved for a runner-level fault (e.g. uncaught exception in the dispatcher itself or the consecutive-`disk_full` threshold), not for per-item failures.
- **Worktree-cleanup failure**: on the happy path the worktree is removed in a finally-block (`git worktree remove --force` then `shutil.rmtree`). If cleanup itself fails (e.g. another process still holds a file handle on Windows), the runner logs the failure and proceeds — the item's status is **not** rolled back to `failed`, the candidate is still recorded. The next startup's `sweep_stale_worktrees` will pick the directory up. Tested by `test_worktree_cleanup_after_run_completes` and `test_worktree_cleanup_on_subprocess_failure` in `api/tests/test_worktree_cleanup.py`.
...
- **Cancel mid-run**: `POST /runs/{run_id}/cancel` flips the run row to `cancelled` and updates every `generation_run_items.status` that is `pending` to `cancelled` in one transaction; the dispatcher's per-iteration check (`SELECT status FROM generation_runs WHERE run_id=?` between sentences) sees the flip and stops dispatching. In-flight worktree subagents drain (Security: don't throw away work already underway); subagents that finish before drain commit as `completed`, the rest commit as `cancelled` with the in-flight subprocess's actual outcome ignored.
...
- **`claude` CLI subprocess hang (no exit, no output)**: each subprocess invocation is bounded by `MAX_WALL_CLOCK_SECONDS_PER_SENTENCE` (default 300s) via `subprocess.Popen.communicate(timeout=...)`; on `TimeoutExpired` the process group is killed (`os.killpg(SIGKILL)` on POSIX) and a 5-second grace `communicate` swallows any tail output. The item is marked `failed` with `error_code='timeout'`. A hung CLI cannot wedge a sentence past the cap, and the killed subprocess can't keep a worktree's files open past cleanup.
...
- **Worktree subagent invocations**: **no automatic retry, no backoff**. The Security section pinned this — a quiet retry loop multiplies subscription pressure. A failed sentence is surfaced to the UI; the user clicks "Retry this sentence" which dispatches a fresh single-sentence run with `parent_run_id`. Whatever the `claude` CLI does internally for transient HTTP failures is its own concern; we do not wrap it in our own retry loop.
...
- **Dispatcher cancel-check granularity**: between every sentence dispatch the runner re-reads `generation_runs.status`; if `cancelled`, it stops dispatching new work but allows already-launched in-flight subagents to drain into `completed` or `cancelled` per the in-flight outcome.
- **Concurrency cap interaction with cancel**: `MAX_CONCURRENT_WORKTREES` (Security §abuse) caps the in-flight set via the `asyncio.Semaphore` around per-sentence tasks; on cancel the drain finishes at most `MAX_CONCURRENT_WORKTREES` extra items. This is the bound the cancel-stops-dispatch test asserts (Security test #17 / Reliability test below).
...
- **Recovery model**: the durable source of truth is `fixtures/` (in git); the runtime store is `data/bible_study.db` (gitignored). On total disk loss, recovery is `git clone` → `make import` → re-run rankings / overlays / generation runs from scratch. Source texts and candidates are reproducible (same fixtures, same prompts, same model — modulo Claude Code subagent non-determinism); rankings / overlays / notes / tie-breaks / hidden combos are **not** reproducible and represent Gavin's authored work.
...
- **Structured JSON logs to stdout**, one line per event. Required keys on every line: `ts` (ISO-8601 UTC), `level`, `event` (a stable identifier — see event list below), `request_id` (UUID4 generated by middleware on every inbound request, propagated via contextvar so service-layer logs carry it). Optional keys correlate the line to domain objects: `run_id`, `sentence_id`, `candidate_id`, `ordinal`, `route`, `status_code`, `duration_ms`, `error_code`. `error_message` may appear; subagent stderr / unparseable stdout is hashed not logged in full (Security §logging discipline).
- **Stable event identifiers** (the load-bearing log-grep surface). Currently emitted by the code: `run.created`, `run.dispatch.started`, `run.item.started`, `run.item.completed`, `run.item.failed`, `run.item.skipped_hidden`, `run.finished`, `run.failed`, `worktree.sweep`, `import.started`, `import.committed`, `import.aborted`. To-be-emitted as the runner / builder / backup paths fill in: `run.dispatch.cancelled`, `run.interrupted`, `run.resumed`, `import.orphans_detected`, `build.started`, `build.completed`, `build.failed`, `backup.completed`, `backup.failed`, `occ.conflict`, `db.busy`. The importer pair uses SQL transaction verbs (`committed` / `aborted`) deliberately — they are precise about whether the SQLite transaction reached COMMIT, and the `aborted` event always carries a `stage` key (`preflight | transaction`) so the on-call grep can distinguish pre-transaction failures (hash mismatch, missing manifest) from in-transaction rollbacks. Adding an event is a code change; renaming one breaks log queries — that's the discipline. **There are no `anthropic.call.*` events** — the worktree mechanism doesn't make HTTP Anthropic calls; per-sentence work is captured by `run.item.started` / `run.item.completed` / `run.item.failed`, and per-subprocess detail (timing, exit code) is folded into those events as `extra` fields.
- **Metrics** (recorded as structured-log lines under `event=metric.<name>`; no Prometheus, no StatsD — overkill for one user). Names and units pinned:
  ...
  - `metric.run.item.latency_ms` (per item, on completion or failure) — labelled with `model`, `source_set_id`, `outcome` (`success | timeout | invalid_response | disk_full | internal | cancelled`). Replaces the legacy SDK-era `metric.anthropic.call.latency_ms`.
  - `metric.run.item.error_count` (per item, on failure only) — labelled with `error_code`.
...
- **"I cancelled a run but the UI still shows in-flight calls"**: expected up to `MAX_CONCURRENT_WORKTREES` items per the in-flight drain. RTO: bounded by `MAX_WALL_CLOCK_SECONDS_PER_SENTENCE` (default 300s); typical subagent latency is ~30s. User action: wait; the runner emits the terminal event when drain completes.
...
- `api/tests/test_runs_failure_modes.py::test_timeout_marks_item_failed_with_timeout_error_code` — fake spawner sleeps past `MAX_WALL_CLOCK_SECONDS_PER_SENTENCE`, the runner kills the subprocess group, assert the item is `failed` with `error_code='timeout'` within the cap + slack. **Invariant: hung-subprocess guard.**
- `api/tests/test_runs_failure_modes.py::test_invalid_response_marks_item_failed` — fake spawner returns empty stdout (or unparseable JSON for a `output_format: json` prompt); assert `error_code='invalid_response'` and no `claude_candidates` row was written. **Invariant: invalid-response code path.**
- `api/tests/test_runs_failure_modes.py::test_internal_error_marks_item_failed` — fake spawner raises `WorktreeGenerationError("internal", ...)`; assert `error_code='internal'` and the run continues. **Invariant: per-item failures don't fail the run.**
- `api/tests/test_runs_failure_modes.py::test_per_sentence_transactions_isolate_failures` — mid-run failure on item k doesn't roll back items 1..k-1's commits; their candidates remain in `claude_candidates`. **Invariant: per-sentence transaction boundary.**
- `api/tests/test_runs_failure_modes.py::test_source_snapshot_dedup_via_content_addressing` — two regenerations of the same `(sentence, prompt, source_set, model)` produce two `claude_candidates` rows but only one `source_snapshots` row. **Invariant: content-addressed snapshot dedup.**
- `api/tests/test_runs_failure_modes.py::test_hidden_combo_skips_subagent_call` — pre-seed a `hidden_combos` row matching the run's combo; assert the runner emits `item_cancelled` (reason `hidden_combo_skipped`) and **does not spawn a subagent**. **Invariant: hidden-combo short-circuit before spawn.**
- *(to-be-added with the cancel-resume slice)* `test_three_consecutive_disk_full_fails_run` — fake-inject SQLite disk-full on three sentence inserts in a row; assert each item has typed `error_code='disk_full'`; assert run-level `status='failed'` after the third. **Invariant: disk-full damage bounding via typed `error_code`.**
- *(to-be-added with the cancel-resume slice)* `test_cancel_stops_dispatch_drains_in_flight` — start a 10-sentence run with `MAX_CONCURRENT_WORKTREES=2`, cancel after item 3 starts, assert at most `3 + 2` items reach `running`/`completed`, the rest are `cancelled`. **Invariant: cancel-stops-dispatch with bounded in-flight drain.**
- *(to-be-added with the cancel-resume slice)* `test_resume_creates_new_run_with_parent_run_id` — cancel a run mid-flight, resume, assert new `run_id` ≠ original, `parent_run_id` set, dense `1..M` ordinals over the unfinished slice only.
- *(to-be-added with the cancel-resume slice)* `test_resume_with_no_unfinished_returns_409` — resume a fully-completed run, assert `409 nothing_to_resume`.
- `api/tests/test_worktree_cleanup.py::test_worktree_cleanup_after_run_completes` — happy-path subagent completes; assert `data/worktrees/{run_id}/{ordinal}/` is gone and the throwaway branch `gen/{run_id}/{ordinal}` no longer exists. **Invariant: worktree hygiene on success.**
- `api/tests/test_worktree_cleanup.py::test_worktree_cleanup_on_subprocess_failure` — fake spawner raises mid-invocation; assert the worktree directory is still removed (finally-block discipline). **Invariant: worktree hygiene on failure.**
- `api/tests/test_worktree_cleanup.py::test_stale_worktree_swept_on_startup` — pre-seed a stale `data/worktrees/{run_id}/` directory, run startup; assert the directory is removed. **Invariant: startup sweep recovers from a prior crash.**
- `api/tests/test_worktree_cleanup.py::test_sweep_is_noop_on_clean_tree` — startup sweep on an empty `data/worktrees/`; assert no error and no log noise.
...
- `api/tests/test_observability.py::test_subagent_error_text_not_in_logs` — fake spawner raises `WorktreeGenerationError("invalid_response", "<long stderr>")`; assert the stderr text never appears in stdout logs (only its hash, per Security §logging discipline).
- `api/tests/test_observability.py::test_metric_events_emitted_for_run` — run a 3-sentence run, assert `metric.run.duration_ms`, `metric.run.item_count`, and three `metric.run.item.latency_ms` events appear with the documented label set (`model`, `source_set_id`, `outcome`).
...
- **Concurrency / fault injection**: the OCC tests above use real `aiosqlite` against a temp file (not a mock) — Postgres-equivalent fidelity isn't applicable since SQLite is the target. The cancel-during-in-flight test uses a real `asyncio` runner with a fake worktree spawner returning controlled latencies so the in-flight count is observable. The disk-full tests use `unittest.mock.patch` on the SQLite connection's `execute` to raise `OperationalError`; these are unit-level and don't touch the real filesystem.
- **Open coverage gaps** (acknowledged, named so they don't drift):
  - **Real `claude` CLI behaviour**: tests use the fake spawner (`api/tests/fakes.py`); we don't exercise the real CLI in CI. Acceptable for v1 — a contract test against the spawner protocol's documented failure types is what we get. Manual smoke-testing in `make dev` against a real Claude Code subscription is the integration check.
...
CREATE TABLE claude_candidates (
  ...
  model TEXT NOT NULL,                      -- worktree-detected model+effort string, e.g. "claude-opus-4-7+xhigh" or "claude-code-2.1.126" (CLAUDE.md §Generation mechanism — the runner currently parses `claude --version` and stores the CLI-version string; once the parallel runner change to pass `--model claude-opus-4-7 --effort xhigh` lands, the runner will record the requested model+effort tuple here instead)
...
CREATE TABLE generation_runs (
  ...
  source_set_id TEXT NOT NULL,
  model TEXT NOT NULL,
  estimated_worktree_count INT NOT NULL,    -- N sentences in resolved scope at run-creation time (post-0002 migration)
...
**`estimated_worktree_count` is just N sentences**: Security §abuse caps generation by **run count** (`MAX_RUNS_PER_DAY`), not summed cost — the `daily-run-count` aggregation is `COUNT(*)` over `idx_gr_created_at`, not `SUM`. The integer-only typing rule from Architect §source_snapshots still applies; the column simply records the resolved scope size at run-creation time so the UI can surface it without re-expanding the `RunScope`. The migration `0002_runs_no_dollar_cost` renamed the column from the SDK-era `estimated_cost_usd_x10000` and dropped the companion `estimated_input_units` column — historical rows' values are now meaningless USD-x10000 numbers and are left as-is (single-user dev DB; no production data to migrate).
...
| Daily run-count cap aggregate | `SELECT COUNT(*) FROM generation_runs WHERE created_at >= ?` | `idx_gr_created_at` | <5 ms |
...
- `api/tests/test_integrity.py::test_estimated_worktree_count_aggregate_uses_integer_arithmetic` — seed 1000 runs with `estimated_worktree_count=1`; assert `SUM` (and `COUNT(*)`) are exactly 1000. Defensive — the v1 cap aggregate is `COUNT(*)`, but the column is still integer-typed by the canonicalisation rule.
